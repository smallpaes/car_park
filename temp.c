...
bool is_unique_park(park pks[POSSIBILITIES], int p_nums, park pk)
{
  ...
  for (int i = 0; i < p_nums; i++)
  {
    ...
    for (int j = 0; j < pk.vehicles_num; j++)
    {
      // if (i < 3)
      // {
      //   printf("Car: %c, i: %i, j: %i \n", FIRST_VEHICLE + j, pk.vehicles[j].head[0], pk.vehicles[j].head[1]);
      //   printf("x: %i, %i \n", pk.vehicles[j].head[0], pks[i].vehicles[j].head[0]);
      //   printf("y: %i, %i \n", pk.vehicles[j].head[1], pks[i].vehicles[j].head[1]);
      //   printf("exit: %i, %i \n", pk.vehicles[j].is_exited, pks[i].vehicles[j].is_exited);
      // }
    ...
    // printf("in is unique: %i\n", is_unique);
    if (!is_unique)
    {
      // printf("is not unique at idx: %i\n", i);
      return false;
    }
  }
  // printf("in is unique return true \n");
  return true;
}
...
int explore_parks(initial_park *ip, park pks[POSSIBILITIES])
{
  ...
  {
    // printf("Possi: %i\n", ip->possibilities);
    // print_park(ip->p, pks[ip->cur_idx], ip->h, ip->w);

    // if (cur_idx == 28 || pks[cur_idx].origin == 28)
    // {
    //   printf("out: cur idx: %i, possi: %i, steps: %i, ori: %i \n", cur_idx, possibilities, pks[cur_idx].steps, pks[cur_idx].origin);
    // printf("idx: %i \n", ip->cur_idx);
    // printf("Car A i: %i, j: %i\n", pks[ip->cur_idx].vehicles[0].head[0], pks[ip->cur_idx].vehicles[0].head[1]);
    // print_park(ip->p, pks[ip->cur_idx], ip->h, ip->w);
    // }

    if (is_finished_park(pks[ip->cur_idx]))
    {
      // printf("%i moves, cur idx: %i, possi: %i \n", pks[cur_idx].steps, cur_idx, possibilities);
      // printf("idx: %i \n", cur_idx);
      // print_path(p, h, w, pks, pks[cur_idx], cur_idx);
      ...
      // return pks[cur_idx].steps;
    }
    for (int i = 0; i < pks[ip->cur_idx].vehicles_num; i++)
    {
      // printf("i: %i, j: %i \n", pks[cur_idx].vehicles[i].head[0], pks[cur_idx].vehicles[i].head[1]);
      if (pks[ip->cur_idx].vehicles[i].is_exited == false)
      {
        ...
        // if (cur_idx == 187)
        // {
        //   printf("Get options: 187, possi: %i \n", possibilities);
        //   print_park(p, pks[187], h, w);
        //   print_park(p, pks[193], h, w);
        // }
        get_options(ip, pks, i);
        // printf("Done explore possi: %i \n", possibilities);
      }
    }
    ip->cur_idx++;
    // printf("done cur idx: %i, possi: %i \n", cur_idx, possibilities);
  }
  // printf("has found: %i \n", has_found);
  // for (int i = 0; i < possibilities; i++)
  // {
  //   printf("idx: %i, Steps: %i, from: %i \n", i, pks[i].steps, pks[i].origin);
  //   print_park(p, pks[i], h, w);
  // }
  if (ip->has_solution)
  {
    print_path(*ip, pks);
...
void get_options(initial_park *ip, park pks[POSSIBILITIES], int v_i)
{
  ...
  if (v.orient == horizontal)
  {
    ...
    // move right
    v.head[1] = old_j + 1;
    if (is_valid_move(*ip, pks[ip->cur_idx], v, v_i))
    {
      // if (ip->cur_idx == 28)
      // {
      //   printf("Right \n");
      ...
      new_p.vehicles[v_i].head[1] = v.head[1];
      // TODO: handle bool return
      handle_at_exit(*ip, &new_p, v, v_i);
      if (is_unique_park(pks, ip->possibilities, new_p))
      {
        // printf("Car: %c, exit: %i\n", FIRST_VEHICLE + v_i, new_p.vehicles[v_i].is_exited);
        // printf(">>idx: %i \n", ip->possibilities);
        new_p.origin = ip->cur_idx;
        new_p.steps += 1;
        pks[ip->possibilities] = new_p;
        ip->possibilities += 1;
        // ip->possibilities += 1;
      }
    }
  ...
  else
  {
    ...
    if (is_valid_move(*ip, pks[ip->cur_idx], v, v_i))
    {
      // printf("is valid down \n");
      new_p.vehicles[v_i].head[0] = v.head[0];
      // TODO: handle bool return
      handle_at_exit(*ip, &new_p, v, v_i);
      if (is_unique_park(pks, ip->possibilities, new_p))
      {
...
bool handle_at_exit(initial_park ip, park *pk, vehicle v, int v_i)
{
  // TODO: Refactor needed: repetitive
  if (v.orient == horizontal)
  {
    // printf("Exit horizontal, car: %c, new j: %i \n", FIRST_VEHICLE + v_i, v.head[1]);
    if (v.head[1] == 0 || v.head[1] + v.length - 1 == ip.w - BORDER_WIDTH)
    {
      pk->vehicles[v_i].is_exited = true;
      // printf("is exit: %i \n", pk->vehicles[v_i].is_exited);
      return true;
    }
  }
  else
  {
    if (v.head[0] == 0 || v.head[0] + v.length - 1 == ip.h - BORDER_WIDTH)
    {
      pk->vehicles[v_i].is_exited = true;
      return true;
    }
...
bool is_valid_move(initial_park ip, park pk, vehicle v, int v_i)
{
  ...
  // TODO: head = v.orient == horizontal ? v.head[1] : v.head[0];
  if (v.orient == horizontal)
  {
    // printf("move horizontal, j: %i, %c \n", v.head[1], p[v.head[0]][v.head[1]]);
    if (v.head[1] == 0 && ip.p[v.head[0]][v.head[1]] == BOLLARD)
    {
      return false;
    }
    // if (v_i == 2 && pk.origin == 19)
    // {
    //   printf("C Passed 1 \n ");
    // }
    // should exclude current index
    if ((v.head[1] + v.length - 1) == (ip.w - BORDER_WIDTH) && ip.p[v.head[0]][v.head[1] + v.length - 1] == BOLLARD)
    {
      return false;
    }
    // if (v_i == 2 && pk.origin == 19)
  ...
  if (v.orient == vertical)
  {
    if (v.head[0] == 0 && ip.p[v.head[0]][v.head[1]] == BOLLARD)
    {
      return false;
    }
    // should exclude current index
    if ((v.head[0] + v.length - 1) >= (ip.h - BORDER_WIDTH) && ip.p[v.head[0] + v.length - 1][v.head[1]] == BOLLARD)
    {
      return false;
    }
  }
  ...
  for (int i = 0; i < pk.vehicles_num; i++)
  {
    ...
    {
      // printf("A i: %i, j: %i, lng: %i, ori: %i \n", v.head[0], v.head[1], v.length, v.orient);
      // printf("B i: %i, j: %i, lng: %i, ori: %i \n", cur.head[0], cur.head[1], cur.length, cur.orient);

      // if (v_i == 2 && pk.origin == 19)
      // {
      //   printf("C In, cmp to %c, i: %i, j: %i, lng: %i, ori: %i \n ", FIRST_VEHICLE + i, pk.vehicles[i].head[0], pk.vehicles[i].head[1], pk.vehicles[i].length, pk.vehicles[i].orient);
      // }
...
void test(void)
{
  ...
  // char validate_1v_1[PARK_MAX_SIDE][PARK_MAX_SIDE] = {{BOLLARD, BOLLARD, BOLLARD, BOLLARD},
  //                                                    {BOLLARD, 'A', GAP, BOLLARD},
  //                                                    {BOLLARD, 'A', GAP, GAP},
  //                                                    {BOLLARD, GAP, BOLLARD, BOLLARD}};
  ...
  assert(validate_park(ip2) == false);
